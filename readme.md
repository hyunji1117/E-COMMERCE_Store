# e-커머스 쇼핑몰 플랫폼
Link: [홈페이지(PC Ver. 지원가능)](https://taupe-lokum-090ebf.netlify.app/, "바로가기")
...
### 2-2.로딩 화면 추가:  
  - 로딩 중인 상태를 직관적으로 표시하기 위해 프로그래스 로딩 화면 구현
  - JavaScript와 SCSS로 부드러운 페이드아웃 애니메이션 추가

### 2-3.PC 환경(전체 화면)에 알맞는 디자인:  
...
## 4.트러블슈팅
  ...
  기능 설명:  
  로딩 화면은 초기 페이지 로딩 시간을 시각적으로 표시하여 사용자가 웹사이트가 준비 중임을 인지하도록 함.  
  `%`로 현재 로딩 진행 상황을 표시하여 직관적 경험 제공.  
  PC와 모바일 환경에서 각각 다른 크기로 반응형 UI 제공.  
